<template>
    <div>
        ...
        <el-card shadow="never" class="border-0">
            <Search @search="getData" @reset="resetSearchForm" :searchForm="searchForm">
                ...
                <template #show>
                    <SearchItem label="商品分类">
                        <el-select v-model="searchForm.category_id" size="default" placeholder="请选择商品分类" clearable>
            ...
            <!-- --------------------------------------------------------- -->
            <!-- 新增|刷新 -->
            <ListHeader layout="create,getList" @create="handleCreate" @get-list="getData">
                <template #default>
                    <el-button type="danger" size="small" @click="handleMultDelete"
                    ...
                    <el-button size="small" @click="handleMultStatusChange(1)">上架</el-button>
                    <el-button size="small" @click="handleMultStatusChange(0)">下架</el-button>
                </template>
            </ListHeader>

            <el-table ref="tableRef" @selection-change="handleSelectionChange" :data="tableData" stripe
                    ...
                    <template #default="scope">
                        <div v-if="searchForm.tab != 'delete'">
                            ...
                            (scope.row.sku_type == 1 && !scope.row.goods_skus.length) ? 'danger' : 'primary'"
                                @click="handleSetGoodsSkus(scope.row)" :loading="scope.row.skusLoading">
                                商品规格</el-button>
                            ...
                            <!-- 轮播图绑定每个对象的bannersLoading 自定义加载
                             handleSetGoodsBanners打开drawer抽屉表单  跳转到banners组件中
                            -->

                            <el-button class="px-1" size="small" :type="!scope.row.content ? 'danger' : 'primary'" text
        ...
        <banners ref="bannersRef" @refresh="getData" />
        <content ref="contentRef" @refresh="getData" />
        <skus ref="skusRef" @refresh="getData" />
    </div>
...
//回收站恢复  删除 封装代码
function useMultiAction(func:Function,msg:string){
  loading.value = true
  func(multSelectionIds.value)
  .then((res:any)=>{
      toast('成功',msg)
      // 清空选中
      if (tableRef.value) {
